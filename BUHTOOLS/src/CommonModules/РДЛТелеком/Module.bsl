...
Процедура ЗаписатьДанныеРДЛ(ДанныеРДЛ)
	...
	Для Каждого Строка ИЗ ДанныеРДЛ Цикл
		...
		МенеджерЗаписи.PaymentTime = РДЛТелеком.ПреобразоватьДатуJson(Строка.PaymentTime);
		Попытка
			МенеджерЗаписи.RefundTime = РДЛТелеком.ПреобразоватьДатуJson(Строка.RefundTime);
		Исключение
		КонецПопытки;
		Попытка
...
Процедура ОтправитьУведомлениеОбОшибке(Ошибка,ДатаНачала,ДатаОкончания) Экспорт
	...
	//Сообщение.Получатели.Добавить("akardan@ufs-online.ru");
