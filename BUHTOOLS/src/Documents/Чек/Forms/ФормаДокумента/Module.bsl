...
Процедура ПрочитатьЧек(Команда)
	...
	НачатьПомещениеФайлаНаСервер(ОбработкаОкончанияПомещения, , , , ,УникальныйИдентификатор);
...
Процедура РазобратьФайл(АдресВХранилище)
	...
	Если СтрокаТекста <> Неопределено Тогда
		...
		Если СокрЛП(ВидОперации) = "Отчёт о регистрации" Тогда
			ЭтотОбъект.ВидЧека = Перечисления.ВидыЧеков.ОтчётРегистрации;
		ИначеЕсли СокрЛП(ВидОперации) = "Отчёт о перерегистрации" Тогда
			ЭтотОбъект.ВидЧека = Перечисления.ВидыЧеков.ОтчетПеререгистрации;	
		Иначе
			ЭтотОбъект.ВидЧека = Перечисления.ВидыЧеков.ОтчётЗакрытиеАрхива;		
		КонецЕсли;
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект,ДанныеЧека);
	Объект.Дата = Дата(ДатаВФормате);
