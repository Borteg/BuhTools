...
Процедура ПолучитьДанныеПровайдеровЗаПериод(АдресВоВременномХранилище) Экспорт
	...
	Соединение = ОбщегоНазначенияSQL.СозданиеПодключениеUFS();
...
Процедура ЗаписатьДанныеПосредников(АдресВХранилище) Экспорт

	СтруктураДанных = ПолучитьИзВременногоХранилища(АдресВХранилище);

	ДокументПосредников = Документы.ДанныеПоПосредникам.СоздатьДокумент();
	ДокументПосредников.Дата = ТекущаяДатаСеанса();
	ДокументПосредников.Заполнить(СтруктураДанных);
	
	ДокументПосредников.Записать(РежимЗаписиДокумента.Проведение);
